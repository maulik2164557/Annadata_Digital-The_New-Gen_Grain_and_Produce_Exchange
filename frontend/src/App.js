...
function AppShell() {
  ...
  const logout = () => { localStorage.removeItem('annadata_token'); localStorage.removeItem('annadata_user'); setUser(null); setMenuOpen(false); navigate('/'); };
  return (
    <div className="app">
      ...
      <div className="od-timeline">
        {STATUS_STEPS.map((step, i) => (
          <div key={step} className={`od-step${i <= stepIndex ? ' done' : ''}${i === stepIndex ? ' current' : ''}`}>
            <div className="od-step-icon">{i < stepIndex || (i === stepIndex && step === 'Delivered') ? '✓' : i === stepIndex ? '●' : i + 1}</div>
            <span>{step}</span>
...
function FarmerReviewsPage() { const user = JSON.parse(localStorage.getItem('annadata_user') || 'null'); const [reviews, setReviews] = useState(null); const [selectedProductId, setSelectedProductId] = useState(null); useEffect(() => { if (user?.role === 'Farmer') request(`/reviews/farmer/${user.id}`).then(r => { setReviews(r); if (r?.products?.length) setSelectedProductId(r.products[0]._id); }); }, [user]); if (!user) return <NavigateToLogin />; if (user.role !== 'Farmer') return <section className="section empty-state-page"><h1>Farmer access<br /><em>required.</em></h1></section>; const products = reviews?.products || []; const selectedProduct = products.find(product => product._id === selectedProductId); return <section className="section reviews-page"><span className="eyebrow">YOUR COMMUNITY VOICE</span><h1>Farmer <em>ratings.</em></h1><div className="rating-summary"><strong>{reviews?.averageRating || user.averageRating || 0}</strong><span>★ average rating<br />{reviews?.count || 0} consumer reviews</span></div><div className="produce-review-list"><h2>Your produce</h2>{products.length ? products.map(product => <button className={`produce-review-item${product._id === selectedProductId ? ' active' : ''}`} key={product._id} onClick={() => setSelectedProductId(product._id)}><span><strong>{product.name}</strong><small>{product.category} · {product.reviews.length} review{product.reviews.length === 1 ? '' : 's'}</small></span><span>→</span></button>) : <div className="empty-state">No produce has been listed yet.</div>}</div>{selectedProduct && <div className="selected-produce-reviews"><div className="section-heading"><div><span className="eyebrow">REVIEWS FOR</span><h2>{selectedProduct.name}</h2></div><span className="selected-produce-date">Harvested {new Date(selectedProduct.harvest_date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</span></div><div className="review-list">{selectedProduct.reviews.length ? selectedProduct.reviews.map(review => <article className="review-card" key={review._id}><strong>{'★'.repeat(review.ratingStars)}{'☆'.repeat(5 - review.ratingStars)}</strong><p>{review.comment}</p><small>By {review.reviewerId?.name || 'Consumer'} · {new Date(review.createdAt).toLocaleDateString('en-IN')}</small></article>) : <div className="empty-state">No consumer reviews for this produce yet.</div>}</div></div>}</section>; }
